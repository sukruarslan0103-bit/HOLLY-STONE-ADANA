...
function gLive() {
  const c = gv('g_c');
  const m = gv('g_um') + gv('g_im');
  const k = c - m;
  const p = c ? (k / c * 100).toFixed(1) : 0;
  ...
  if (c || m) {
    ...
    const ke = document.getElementById('gL_k');
    ke.textContent = fmt(k);
    ke.style.color = k >= 0 ? 'var(--gr)' : 'var(--rd)';

    const pe = document.getElementById('gL_p');
    pe.textContent = '%' + p;
    pe.style.color = p >= 30 ? 'var(--gr)' : p >= 15 ? 'var(--acc)' : 'var(--rd)';
  }
}
...
async function gAdd() {
  const row = {
    ...
    cerez: gv('g_ce'),
    bilet: gv('g_bi'),
    urun_maliyet: gv('g_um'),
    ikram_maliyet: gv('g_im'),
    not_alani: gs('g_no')
  };

  if (!row.tarih) {
    showToast('❌ Tarih seçin', 'err');
    return;
  }
...
function rG() {
  ...
  if (!STATE.g.length) {
    tb.innerHTML = '<tr><td colspan="13" class="empty-row">📅 Kayıt yok</td></tr>';
    return;
  }
  ...
  tb.innerHTML = STATE.g.map(r => {
    ...
    return `<tr>
      <td class="tnum">${r.tarih}</td>
      <td class="tnum">${fmt(r.ciro)}</td>
      <td class="tnum">${fmt(r.nakit)}</td>
      <td class="tnum">${fmt(r.pos)}</td>
      <td class="tnum">${fmt(r.bira)}</td>
      <td class="tnum">${fmt(r.alkol)}</td>
      <td class="tnum">${fmt(r.yiyecek)}</td>
      <td class="tnum">${fmt(r.bilet)}</td>
...
function kmRenderRows() {
  const c = document.getElementById('kmRows');
  if (!c) return;
...
function kmTotal() {
  return Array.from(document.querySelectorAll('.dyn-val'))
...
function kmCalc() {
  ...
  if (!bf) {
    res.innerHTML = '<div class="card nm"><div class="empty-state">⚖️ Bilet fiyatını girin</div></div>';
    document.getElementById('kmTbl').innerHTML = '';
    return;
  }
  ...
  if (net <= 0) {
    res.innerHTML = '<div class="alert a-r">⚠️ Bilet fiyatı maliyetten düşük!</div>';
    return;
  }

  const be = Math.ceil((sb - dg) / net);
  const dol = (be / kap * 100).toFixed(0);
  const maxK = kap * net + dg - sb;
  ...
  if (ba) {
    ...
    h += `<div class="rbox ${k >= 0 ? 'rbox-g' : 'rbox-r'}">
      <div class="rbox-label">${ba} Bilet Satılırsa</div>
  ...
  if (hBilet) {
    h += `<div class="info-box">
      <div class="ib-label">🎯 Hedef Kâr İçin</div>
      <div class="ib-num">${hBilet} BİLET</div>
      <div class="ib-sub">%${(hBilet / kap * 100).toFixed(0)} doluluk → ${fmt(hk)} kâr</div>
    </div>`;
  }
  ...
  document.getElementById('kmTbl').innerHTML = steps.map(a => {
    ...
    return `<tr style="${isB ? 'background:rgba(245,200,66,.06)' : ''}">
      <td class="tnum">${a}${isB ? ' 🎯' : ''}</td>
      <td class="tnum">${(a / kap * 100).toFixed(0)}%</td>
      <td class="tnum">${fmt(bg)}</td>
      <td class="tnum">${fmt(g)}</td>
...
function kzSetTip(t) {
  ...
  if (t === 1) {
    info.innerHTML = '<b>Tip 1:</b> Kaşeyi ve masrafları biz karşılıyoruz, bileti biz satıyoruz.';
    info.className = 'alert a-b';
  } else {
    info.innerHTML = '<b>Tip 2 — Kapı Destek:</b> Merkez turne, biz belirlenen destek tutarını + masrafları karşılıyoruz.';
    info.className = 'alert a-y';
  }
...
function kzGetGelir() {
  if (STATE.kzTip === 1) {
    return (gv('kz_ba') * gv('kz_bf')) + gv('kz_bc') + gv('kz_bic') + gv('kz_sp') + gv('kz_dg');
  }
...
function kzGetGider() {
  const base =
    gv('kz_ka') + gv('kz_ko') + gv('kz_ek') + gv('kz_gu') + gv('kz_ex') +
    gv('kz_kp') + gv('kz_ik') + gv('kz_um') + gv('kz_bm') + gv('kz_re') +
    gv('kz_ul') + gv('kz_di');
...
function kzLive() {
  ...
  if (g || gi) {
    ...
    const ke = document.getElementById('kzL_k');
    ke.textContent = fmt(k);
    ke.style.color = k >= 0 ? 'var(--gr)' : 'var(--rd)';

    const me = document.getElementById('kzL_m');
    me.textContent = '%' + m;
    me.style.color = m >= 20 ? 'var(--gr)' : m >= 5 ? 'var(--acc)' : 'var(--rd)';
  }
}

async function kzSave() {
  const gelir = kzGetGelir();
  const gider = kzGetGider();
  ...
  const row = {
    ...
    not_alani: gs('kz_n'),
    bilet_adet: gv('kz_ba'),
    bilet_fiyat: gv('kz_bf'),
    bar_ciro: STATE.kzTip === 1 ? gv('kz_bc') : gv('kz2_bc'),
    bilet_ciro: gv('kz_bic'),
    sponsor: gv('kz_sp'),
    diger_gelir: STATE.kzTip === 1 ? gv('kz_dg') : gv('kz2_dg'),
    destek_tutari: gv('kz2_de'),
    kase: gv('kz_ka'),
    konser_diger_gider: gv('kz_ko'),
    ekipman: gv('kz_ek'),
    guvenlik: gv('kz_gu'),
    ekstra_personel: gv('kz_ex'),
    kadrolu_personel: gv('kz_kp'),
    ikram_maliyet: gv('kz_ik'),
    urun_maliyet: gv('kz_um'),
    bilet_maliyet: gv('kz_bm'),
    reklam: gv('kz_re'),
    ulasim: gv('kz_ul'),
    diger_gider: gv('kz_di'),
    toplam_gelir: gelir,
    toplam_gider: gider,
    net_kar: gelir - gider
  };
  ...
  if (!row.sanatci) {
    showToast('❌ Sanatçı adı girin', 'err');
    return;
  }
...
function kzClr() {
  ...
  const fields = [
    'kz_s','kz_ba','kz_bf','kz_bc','kz_bic','kz_sp','kz_dg','kz2_bc','kz2_dg',
    'kz2_de','kz_ka','kz_ko','kz_ek','kz_gu','kz_ex','kz_kp','kz_ik','kz_um',
    'kz_bm','kz_re','kz_ul','kz_di','kz_n'
  ];
...
function kzEdit(id) {
  const r = STATE.kz.find(x => x.id === id);
  if (!r) return;
  ...
  sv('kz_d', r.durum);
  sv('kz_n', r.not_alani || '');
  sv('kz_ba', r.bilet_adet || '');
  sv('kz_bf', r.bilet_fiyat || '');
  sv('kz_bic', r.bilet_ciro || '');
  sv('kz_sp', r.sponsor || '');

  if (r.tip === 2) {
    sv('kz2_bc', r.bar_ciro || '');
    sv('kz2_dg', r.diger_gelir || '');
    sv('kz2_de', r.destek_tutari || '');
  } else {
    sv('kz_bc', r.bar_ciro || '');
    sv('kz_dg', r.diger_gelir || '');
  }

  sv('kz_ka', r.kase || '');
  sv('kz_ko', r.konser_diger_gider || '');
  sv('kz_ek', r.ekipman || '');
  sv('kz_gu', r.guvenlik || '');
  sv('kz_ex', r.ekstra_personel || '');
  sv('kz_kp', r.kadrolu_personel || '');
  sv('kz_ik', r.ikram_maliyet || '');
  sv('kz_um', r.urun_maliyet || '');
  sv('kz_bm', r.bilet_maliyet || '');
  sv('kz_re', r.reklam || '');
  sv('kz_ul', r.ulasim || '');
  sv('kz_di', r.diger_gider || '');
...
function rKZ() {
  ...
  if (!STATE.kz.length) {
    tb.innerHTML = '<tr><td colspan="13" class="empty-row">🎤 Konser kaydı yok</td></tr>';
    return;
  }
...
function uLive() {
  ...
  const bk = s - m;
  const mj = s ? (bk / s * 100).toFixed(1) : 0;
...
async function uAdd() {
  const row = {
    ad: gs('u_ad'),
    kategori: gs('u_kt'),
    satis_fiyati: gv('u_sf'),
    maliyet: gv('u_ml'),
    aylik_adet: gv('u_ad2')
  };

  if (!row.ad) {
    showToast('❌ Ürün adı girin', 'err');
    return;
  }
...
function rU() {
  ...
  if (!STATE.u.length) {
    tb.innerHTML = '<tr><td colspan="10" class="empty-row">🍺 Ürün eklenmedi</td></tr>';
    return;
  }

  const sorted = [...STATE.u].sort((a, b) =>
    (b.satis_fiyati - b.maliyet) * b.aylik_adet - (a.satis_fiyati - a.maliyet) * a.aylik_adet
  );

  const maxK = Math.max(...sorted.map(r => (r.satis_fiyati - r.maliyet) * r.aylik_adet), 1);

  tb.innerHTML = sorted.map(r => {
    const bk = r.satis_fiyati - r.maliyet;
    const m = r.satis_fiyati ? (bk / r.satis_fiyati * 100).toFixed(1) : 0;
    const ak = bk * r.aylik_adet;
    const bw = Math.round(ak / maxK * 100);

    return `<tr>
      <td><b>${r.ad}</b></td>
      <td class="tmut">${r.kategori}</td>
      <td class="tnum">${fmt(r.satis_fiyati)}</td>
      <td class="tnum tneg">${fmt(r.maliyet)}</td>
      <td class="tnum tpos">${fmt(bk)}</td>
      <td>${chip('%' + m, m >= 50 ? 'cg' : m >= 30 ? 'cy' : 'cr')}</td>
      <td class="tnum">${r.aylik_adet}</td>
      <td class="tnum tpos">${fmt(ak)}</td>
  ...
  if (el) {
    el.innerHTML = sorted.slice(0, 6).map(r => {
      ...
      return `<div class="pb">
        <div class="pb-l">
          <span>${r.ad} <span class="tmut">${r.kategori}</span></span>
          <span style="font-family:var(--fm);color:${m >= 50 ? 'var(--gr)' : m >= 30 ? 'var(--acc)' : 'var(--rd)'}">${m}%</span>
        </div>
        <div class="pb-bar"><div class="pb-fill" style="width:${m}%;background:${m >= 50 ? 'var(--gr)' : m >= 30 ? 'var(--acc)' : 'var(--rd)'}"></div></div>
      </div>`;
...
async function ayAdd() {
  ...
  const row = {
    ay: parseInt(gs('ay_ay')),
    yil: parseInt(gs('ay_yl')) || 2026,
    ciro: ci,
    urun_maliyet: um,
    ikram_maliyet: ik,
    diger_giderler: g,
  ...
  try {
    ...
    if (idx >= 0) STATE.ay[idx] = saved;
    else STATE.ay.unshift(saved);
    rAy();
    rAna();
    showToast('✅ Kaydedildi');
  } catch (e) {
...
function rAy() {
  ...
  if (!STATE.ay.length) {
    tb.innerHTML = '<tr><td colspan="10" class="empty-row">📊 Kayıt yok</td></tr>';
    return;
  }

  tb.innerHTML = [...STATE.ay].sort((a, b) => b.yil - a.yil || b.ay - a.ay).map(r => {
    const bp = (r.brut_marj * 100).toFixed(1);
    const np = (r.net_marj * 100).toFixed(1);
    const gc = r.ciro ? (r.toplam_gider / r.ciro * 100).toFixed(0) : 0;

    return `<tr>
      <td><b>${AYLAR[r.ay]} ${r.yil}</b></td>
      <td class="tnum">${fmt(r.ciro)}</td>
      <td class="tnum tneg">${fmt(r.urun_maliyet + r.ikram_maliyet)}</td>
      <td class="tnum tneg">${fmt(r.toplam_gider)}</td>
      <td class="${r.net_kar >= 0 ? 'tpos' : 'tneg'}">${fmt(r.net_kar)}</td>
...
function rGi() {
  ...
  if (!STATE.gi.length) {
    tb.innerHTML = '<tr><td colspan="6" class="empty-row">💸 Kayıt yok</td></tr>';
    return;
  }
  ...
  tb.innerHTML = STATE.gi.map(r => `<tr>
    ...
    <td class="tnum tneg">${fmt(r.tutar)}</td>
    <td>${r.odeme_yontemi}</td>
...
async function pAdd() {
  ...
  try {
    ...
    STATE.p.push(saved);
    rP();
    ['p_ad', 'p_ma', 'p_sg', 'p_te'].forEach(id => sv(id, ''));
...
function rP() {
  ...
  if (!STATE.p.length) {
    el.innerHTML = '<div class="empty-state">👥 Personel eklenmedi</div>';
    document.getElementById('pMaas').innerHTML = '';
    return;
  }

  el.innerHTML = STATE.p.map((r, i) => {
    const c = PA_COLORS[i % PA_COLORS.length];
    const ini = (r.ad_soyad || '?').split(' ').map(x => x[0]).join('').slice(0, 2).toUpperCase();

    return `<div class="pc">
      <div class="pa" style="background:${c}22;color:${c}">${ini}</div>
      <div class="pi">
        <div class="pi-n">${r.ad_soyad}</div>
        <div class="pi-r">${r.pozisyon} · ${r.tur}</div>
      </div>
      <div class="pr">
        ${r.telefon ? `<span class="tmut" style="font-family:var(--fm)">${r.telefon}</span>` : ''}
  ...
  STATE.p.forEach(r => {
    byT[r.tur] = (byT[r.tur] || 0) + (r.aylik_maas || 0);
  });
  ...
  document.getElementById('pMaas').innerHTML = `
    ...
    <div class="dv"></div>
    ${Object.entries(byT).map(([t, v]) => `
      <div class="pb">
        <div class="pb-l"><span>${t}</span><span style="font-family:var(--fm)">${fmt(v)}</span></div>
        <div class="pb-bar"><div class="pb-fill" style="width:${tm ? (v / tm * 100).toFixed(0) : 0}%;background:var(--acc)"></div></div>
      </div>
    `).join('')}
    <div class="dv"></div>
...
function rAna() {
  ...
  const kpis = [
    ...
    ...(latAy ? [{
      l: AYLAR[latAy.ay] + ' Net Marj',
      v: '%' + (latAy.net_marj * 100).toFixed(1),
      c: latAy.net_marj >= .1 ? 'var(--gr)' : latAy.net_marj >= 0 ? 'var(--acc)' : 'var(--rd)',
      f: AYLAR[latAy.ay] + ' ' + latAy.yil
    }] : [])
  ];
  ...
  mkChart('cA7', 'bar', {
    labels: s7.map(r => r.tarih.slice(5)),
    datasets: [{
      label: 'Ciro',
      data: s7.map(r => r.ciro),
      backgroundColor: STATE.cfg.accent + '44',
      borderColor: STATE.cfg.accent,
      borderWidth: 2,
      borderRadius: 4
    }]
  });
  ...
  if (aS.length) {
    mkChart('cAT', 'bar', {
      labels: aS.map(r => AYLAR[r.ay].slice(0, 3)),
      datasets: [
        {
          label: 'Ciro',
          data: aS.map(r => r.ciro),
          backgroundColor: STATE.cfg.accent + '33',
          borderColor: STATE.cfg.accent,
          borderWidth: 1.5,
          borderRadius: 3
        },
        {
          label: 'Net Kar',
          data: aS.map(r => r.net_kar),
          backgroundColor: r => r.raw >= 0 ? 'rgba(46,204,138,.4)' : 'rgba(232,80,58,.4)',
          borderColor: r => r.raw >= 0 ? '#2ecc8a' : '#e8503a',
          borderWidth: 1.5,
          borderRadius: 3
        }
      ]
  ...
  if (STATE.kz.length) {
    kEl.innerHTML = `<table><thead><tr><th>Sanatçı</th><th>Tip</th><th>Tarih</th><th>Kar/Zarar</th></tr></thead><tbody>` +
      [...STATE.kz].sort((a, b) => b.tarih.localeCompare(a.tarih)).slice(0, 5).map(r => {
        return `<tr>
          <td><b>${r.sanatci || '—'}</b></td>
          <td>${r.tip === 2 ? chip('Kapı', 'co') : chip('Kaşe', 'cb')}</td>
          <td class="tnum tmut">${r.tarih}</td>
          <td class="${r.net_kar >= 0 ? 'tpos' : 'tneg'}">${fmt(r.net_kar)}</td>
        </tr>`;
      }).join('') + '</tbody></table>';
  } else {
    kEl.innerHTML = '<div class="empty-state">🎤 Konser kaydı yok</div>';
  }
...
function rGrafik() {
  ...
  if (aS.length) {
    ...
    mkChart('cMj', 'line', {
      ...
      datasets: [
        {
          label: 'Brüt %',
          data: aS.map(r => +(r.brut_marj * 100).toFixed(1)),
          borderColor: '#2ecc8a',
          backgroundColor: 'rgba(46,204,138,.1)',
          fill: true,
          tension: .4,
          pointRadius: 4
        },
        {
          label: 'Net %',
          data: aS.map(r => +(r.net_marj * 100).toFixed(1)),
          borderColor: STATE.cfg.accent,
          backgroundColor: STATE.cfg.accent + '18',
          fill: true,
          tension: .4,
          pointRadius: 4
        }
    ...
    clrChart('cTr', 'Aylık veri yok');
    clrChart('cMj', 'Aylık veri yok');
  }

  if (STATE.gi.length) {
    const cm = {};
    STATE.gi.forEach(g => {
      cm[g.kategori] = (cm[g.kategori] || 0) + (g.tutar || 0);
    });

    const cats = Object.entries(cm).sort((a, b) => b[1] - a[1]).slice(0, 8);

    mkChart('cGD', 'doughnut', {
      labels: cats.map(c => c[0]),
      datasets: [{
        data: cats.map(c => c[1]),
        backgroundColor: ['#f5c842','#2ecc8a','#4f8ef7','#9b72f5','#e8503a','#22d3ee','#f97316','#94a3b8'],
      ...
      plugins: {
        legend: {
          position: 'right',
          labels: { color: '#8e90a8', font: { size: 11 }, padding: 8, boxWidth: 10 }
        }
      }
    });
  } else {
    clrChart('cGD', 'Gider kaydı yok');
  }
  ...
  if (STATE.kz.length) {
    ...
    mkChart('cKB', 'bar', {
      labels: ks.map(r => r.sanatci || '?'),
      datasets: [
        {
          label: 'Gelir',
          data: ks.map(r => r.toplam_gelir),
          backgroundColor: 'rgba(46,204,138,.45)',
          borderColor: '#2ecc8a',
          borderWidth: 2,
          borderRadius: 4
        },
        {
          label: 'Gider',
          data: ks.map(r => r.toplam_gider),
          backgroundColor: 'rgba(232,80,58,.45)',
          borderColor: '#e8503a',
          borderWidth: 2,
          borderRadius: 4
        },
        {
          label: 'Net Kar',
          data: ks.map(r => r.net_kar),
          backgroundColor: r => r.raw >= 0 ? 'rgba(245,200,66,.6)' : 'rgba(232,80,58,.3)',
          borderColor: r => r.raw >= 0 ? STATE.cfg.accent : '#e8503a',
          borderWidth: 2,
          borderRadius: 4
        }
      ]
    ...
    clrChart('cKB', 'Konser kaydı yok');
  }

  if (STATE.u.length) {
    const us = [...STATE.u]
      .sort((a, b) => (b.satis_fiyati - b.maliyet) * b.aylik_adet - (a.satis_fiyati - a.maliyet) * a.aylik_adet)
      .slice(0, 10);

    mkChart('cUB', 'bar', {
      labels: us.map(u => u.ad),
      datasets: [
        {
          label: 'Aylık Ciro',
          data: us.map(u => u.satis_fiyati * u.aylik_adet),
        ...
        {
          label: 'Aylık Kar',
          data: us.map(u => (u.satis_fiyati - u.maliyet) * u.aylik_adet),
          backgroundColor: 'rgba(46,204,138,.45)',
          borderColor: '#2ecc8a',
          borderWidth: 2,
          borderRadius: 4
    ...
    clrChart('cUB', 'Ürün kaydı yok');
  }

  const s14 = [...STATE.g].sort((a, b) => b.tarih.localeCompare(a.tarih)).slice(0, 14).reverse();
  if (s14.length) {
    mkChart('cG14', 'line', {
      labels: s14.map(r => r.tarih.slice(5)),
      datasets: [
        {
          label: 'Ciro',
          data: s14.map(r => r.ciro),
          borderColor: STATE.cfg.accent,
          backgroundColor: STATE.cfg.accent + '18',
          fill: true,
          tension: .4,
        ...
        {
          label: 'Brüt Kar',
          data: s14.map(r => (r.ciro || 0) - ((r.urun_maliyet || 0) + (r.ikram_maliyet || 0))),
          borderColor: '#2ecc8a',
          backgroundColor: 'rgba(46,204,138,.1)',
          fill: true,
          tension: .4,
...
function rRapor() {
  ...
  ${[...STATE.kz].sort((a, b) => b.net_kar - a.net_kar).map(r => {
    const m = r.toplam_gelir ? (r.net_kar / r.toplam_gelir * 100).toFixed(1) : 0;
    return `<tr><td><b>${r.sanatci}</b></td><td>${r.tip === 2 ? chip('Kapı', 'co') : chip('Kaşe', 'cb')}</td><td class="tnum tmut">${r.tarih}</td><td class="tnum tpos">${fmt(r.toplam_gelir)}</td><td class="tnum tneg">${fmt(r.toplam_gider)}</td><td class="${r.net_kar >= 0 ? 'tpos' : 'tneg'}">${fmt(r.net_kar)}</td><td>${chip('%' + m, m >= 20 ? 'cg' : m >= 5 ? 'cy' : 'cr')}</td></tr>`;
  }).join('')}
  </tbody></table></div></div>` : ''}
  ...
  STATE.charts[id] = new Chart(cv, {
    ...
    options: {
      ...
      scales: type === 'doughnut' ? {} : {
        x: {
          grid: { color: 'rgba(44,46,72,.4)' },
          ticks: { color: '#52546a', font: { size: 11 } }
        },
        y: {
          grid: { color: 'rgba(44,46,72,.4)' },
          ticks: { color: '#52546a', font: { size: 11 }, callback: v => fmt(v) }
        }
      },
      ...extra
    }
